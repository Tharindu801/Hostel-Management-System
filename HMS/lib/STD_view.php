<?php
	...
	$sql_basic="SELECT * FROM student_detail WHERE reg_no='".$key."';";
	$sql_Contact="SELECT * FROM student_contact WHERE reg_no='".$key."';";
	$sql_guardient="SELECT * FROM gurdian_detail WHERE reg_no='".$key."';";
	$sql_stdHostel = "SELECT * FROM student_hostel WHERE reg_no = '$key';";
	

	$result_Main = $conn->query($sql_basic);
	$result_Contact = $conn->query($sql_Contact);
	$result_guardient = $conn->query($sql_guardient);
	$result_stdHostel = $conn->query($sql_stdHostel);
...
//get student hostel details
	if($result_stdHostel->num_rows > 0)
	{
		while($row = $result_stdHostel->fetch_assoc())
		{
			$Hid = $row["hostel_id"];
			$RoomNo = $row["room_no"];
		}
