...
include('STD_Con.php');
//Query
	$sql = "SELECT s.reg_no, s.full_Name, s.year_of_study, s.faculty, s.course FROM student_detail as s, student_pic as p, student_hostel as h WHERE (s.full_Name LIKE '%".$key."%' OR s.reg_no LIKE '%".$key."%' OR s.faculty LIKE '%".$key."%' OR s.course LIKE '%".$key."%') AND s.reg_no = p.reg_No AND p.name IS NULL AND h.reg_no = s.reg_no AND h.hostel_id = '$hid';";
